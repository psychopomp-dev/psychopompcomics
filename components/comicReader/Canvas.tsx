...
export const Canvas = (props: CanvasProps) => {
    const { canvasRef, width, height, src, alt, layout, objectPosition, page, ...other } = props;
    ...
    useEffect(() => {
        ...
        let ctx: CanvasRenderingContext2D | null | undefined = current?.getContext('2d');
        ...
        comicImage.onload = () => {
            if (ctx && current) {
                ctx.canvas.height = getCanvasDimension(current, Dimension.Height);
                ctx.canvas.width = getCanvasDimension(current,Dimension.Width);
    ...
    return (
        <>
            <StyledCanvas ref={canvasRef} width={width} height={height} onClick={handleOnClick} {...other} />
        </>
    );
};
