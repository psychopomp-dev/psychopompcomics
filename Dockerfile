...
COPY . .
# Production prune must leave sharp install scripts runnable so next/image
# can load the linuxmusl native binary. Explicit platform flags keep
# cross-builds (e.g. arm64 host → amd64) correct.
ENV CI=true
ENV HUSKY=0
RUN pnpm build \
